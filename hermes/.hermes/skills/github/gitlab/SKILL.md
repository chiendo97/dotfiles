...
```bash
uv run ${HERMES_SKILL_DIR}/scripts/gitlab_cli.py <command> [options]
```

### Post an Inline Comment

```bash
uv run ${HERMES_SKILL_DIR}/scripts/gitlab_cli.py inline-comment \
...
```bash
uv run ${HERMES_SKILL_DIR}/scripts/gitlab_cli.py batch-inline --mr 20 \
...
```bash
# All discussions
uv run ${HERMES_SKILL_DIR}/scripts/gitlab_cli.py discussions --mr 20

# Only inline (diff) discussions
uv run ${HERMES_SKILL_DIR}/scripts/gitlab_cli.py discussions --mr 20 --inline-only

# Only unresolved
uv run ${HERMES_SKILL_DIR}/scripts/gitlab_cli.py discussions --mr 20 --unresolved
```

### List Open MRs Across Repos

```bash
uv run ${HERMES_SKILL_DIR}/scripts/gitlab_cli.py mrs
```
...
```bash
uv run ${HERMES_SKILL_DIR}/scripts/gitlab_cli.py reply \
