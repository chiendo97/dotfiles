...
description: Curl-only REST workflow for creating, editing, inspecting, importing, exporting, and refining diagrams on a local mcp_excalidraw canvas server. Use when an agent needs to control an Excalidraw canvas without MCP tools, using curl against EXPRESS_SERVER_URL or http://127.0.0.1:3000. Covers health checks, element CRUD, batch creation, Mermaid conversion, viewport/image requests, snapshots, and JSON import/export. Requires a running canvas server.
---
